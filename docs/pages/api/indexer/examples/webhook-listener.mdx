...
To begin, direct yourself to the [Sequence Builder](https://sequence.build) and follow [this walkthrough for Public API Access Key](/solutions/builder/getting-started#3-optional-for-development-claim-an-api-access-key) and [this walkthrough for a Secret API Access Key](/solutions/builder/getting-started#4-optional-for-development-create-a-service-account), both required for using the Sequence Indexer Webhooks. 
...
Examples
1. [Listen to All Mints of a Contract](/api/indexer/examples/webhook-listener#1-listen-to-all-mints-of-a-contract)
2. [Listen to Mints of a Specific Token ID](/api/indexer/examples/webhook-listener#2-listen-to-a-specific-token-id)
...
### 2. Listen to a Specific Token ID

If you want to get all mints of a specific token ID
...
### 3. Listen to Topic Events for a Contract Address

As an example, listening to all events emitted by a webhook is as simple as inputted the Event Topic with input parameter types.

Here's an example with `ApprovalForAll`

:::code-group

```bash [cURL]
curl -X POST -H "Content-Type: application/json" -H "X-Access-Key: <project_access_key>" -H "Authorization: BEARER <secret_API_key>", "filters": {"contractAddresses":["0x9bec34c1f7098e278afd48fedcf13355b854364a"], events: ['ApprovalForAll(address,address,bool)']}}'
```
...
const req = {
	...
	filters:{   
		contractAddresses: ['0x9bec34c1f7098e278afd48fedcf13355b854364a'],
		events: ['ApprovalForAll(address,address,bool)'],
	}
}
...
const req = {
	...
	filters:{   
		contractAddresses: ['0x9bec34c1f7098e278afd48fedcf13355b854364a'],
		events: ['RequestCreated(uint256,address,address,uint256,bool,uint256,address,uint256,uint256)'],
	}
}
