...
- **at_hash** (string) - Access token hash. A value used to validate the token integrity and ensure it hasn't been tampered with.
...
- **locale** (string) - The locale of the user, representing the user's language and region settings.
...
- **nonce_supported** (boolean) - Whether the nonce claim is supported.
