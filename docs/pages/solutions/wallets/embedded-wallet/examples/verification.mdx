...
The following sequence diagram flows showcase how to both generate and verify a random unique nonce stored in an off-chain db.
...
The following sequence diagram flows showcase how to both generate and verify a timestamped nonce stored in an off-chain db, that allows an application to validate a wallet can be authorized for a given period of time.

### Generate Nonce
[Same sequence diagram as no expiry](/solutions/wallets/embedded-wallet/examples/verification#generate-nonce)
