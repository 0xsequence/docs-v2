...
Sequence Embedded Wallet API is a service that runs on the [AWS Nitro Enclaves](https://aws.amazon.com/ec2/nitro/nitro-enclaves/) platform.
...
The Embedded Wallet Enclave is the only entity able to use the encrypted wallet data. Its security is guaranteed by:
...
3. The enclave code is open source and anyone may audit it and verify that the Embedded Wallet uses the exact audited version of the source code. [Learn more about verification](/solutions/technical-references/embedded-wallet/03-enclave-verification).
