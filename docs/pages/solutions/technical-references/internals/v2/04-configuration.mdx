...
In the context of Sequence Wallets, the "Wallet Configuration" is a set of parameters that defines the behavior of the wallet, this is primarely used to define the access control of the wallet, meaning who can sign transactions and how many signatures are needed.
...
The `threshold` is an `uint16`, it can have any value between 0 and 65535. Signatures can only be considered valid or invalid, if the sum of the weights of the signers that signed the transaction is greater or equal to the threshold, the signature is considered valid.
...
The `tree` is an unbalanced binary Merkle tree, where each leaf may contain a signer, a static signature or a sub-tree. The tree can represent any combination of signers and weights, and can be used to create complex multi-signature wallets.

The possible leaf types are:

#### Signer

Signers are represented by a signer `address` and an `uint8` weight. The weight is how much the signer contributes to the threshold.
The address can belong to either an `ERC1271` compliant contract or to an `EOA` wallet.
...
Notice static subdigests that exists within nested trees will have their "Infinity" weight reduced to the weight of the nested tree.
...
- Non-linear weight distribution, A and B signers can provide 1 weight each, but together the can provide 3 weight.
...
:::warning
The wallet contracts have no way of validating the correctness of the configuration, the responsability of ensuring the configuration is correct is on the SDKs interacting with the contracts.

Things like `threshold == 0` or `threshold > total weight` will result on fully unauthenticated wallets or inaccessible wallets respectively.
:::
...
The tree is not required to be sorted in any way, the `imageHash` is calculated as follows:
...
The `hashTree` function is a recursive function that hashes the tree into a single `bytes32` value, psuedo code for the `hashTree` function is as follows:
...
All Sequence Wallets have an "initial configuration", this is implemented by using the `imageHash` of the initial configuration as the SALT during the `CREATE2` deployment of the wallet.

Wallets are deployed by calling the `deploy` function of the `Factory` contract, this function takes the following parameters:
