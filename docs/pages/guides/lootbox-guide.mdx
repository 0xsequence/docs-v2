...
- [8. Uploading media to Sequence metadata service](/guides/lootbox-guide#8-uploading-media-to-sequence-metadata-service): Upload collection and token metadata to Sequence
...
However, if you plan to only ever mint a single token of the generated media and you prefer technical security in managing parrallized minting over managing the security of the relayer private key (which can always be rotated by setting a new collectible `role` via the [Builder](https://sequence.build/)) an ERC721 might be better suited.

To deploy a contract, you can follow [this guide](/solutions/collectibles/contracts/01-deploy-an-item-collection) to deploy your ERC1155, and update your `wrangler.toml` with the `CONTRACT_ADDRESS`
...
### minting a token
You can follow [this default guide](/guides/mint-collectibles-serverless) on minting a severless gasless minter with your deployed `ERC1155` contract

### add lootbox object composition

Next, ensure that you can mint a token making a request to the API. Then, add the additional object composition design to include the lootbox specific functions, which can be found in a [template lootbox cloudflare relayer](https://github.com/0xsequence-demos/demo-cloudflare-worker-lootbox/tree/template). 

Where in the [completed lootbox code](https://github.com/0xsequence-demos/demo-cloudflare-worker-lootbox/blob/master/proud-darkness-022a/src/index.ts#L505), the `callContract` is basically the same
...
```
and call the `getInferenceStatus` with the `inferenceId` passed in, where when you recieve the response, you can obtain the image url with `resObject.inference.images[0].url`
...
## 8. Uploading media to Sequence metadata service

With our media `url` in hand, we can move onto storing the asset to the `Sequence Metadata Service`

Which can be accomplished with 3 steps
- create collection from a cURL request
- process and store an image URL to Sequence Metadata Service
- publish collection from a cURL request

### create collection from a cURL request

As a one time requirement to uploading media to the service, a collection has to be first made. By using the Secret API Key from [step 2](/guides/lootbox-guide#2-access-key-management) and `projectID` retrieved from the [Builder](https://sequence.build/)
...
--header 'Authorization: Bearer <secrect_API_key>' \
--data '{
...
We then set the `collectionID` from the returned response in the `wrangler.toml`

### process and store an image URL to Sequence Metadata Service

In order to process and store the generated media at the URL, we implement a few steps

- create token
- create asset
- upload asset
- update non-private token
...
### full pagination of the indexer for a day
<Callout type='info'>
