...
<Callout type="info">
  You can play a live version of the game [here](https://0xsequence.github.io/waas-airplane-demo/)

  Where the full code to this game can be found [here](https://github.com/0xsequence/waas-airplane-demo/tree/cloudflare-integration)
...
- [2. Integrate SequenceKit](/guides/webgl-guide#2-integrate-sequencekit): Allow all EOAs and Sequence Wallet to authenticate the user
...
This is accomplished in the code by adding a key/value of `isPlane` to the normal cloudflare request body, and creating an additional `if/else` in the cloudflare relayer.
...
Finally, to burn the achievement token, we can no longer use a cloduflare relayer for actions sent to the blockchain, because when the minting was performed 'on behalf of' the address using the relayer (making the `msg.sender` in the contract one of the `relayer` addresses) for this, we want to make sure the `msg.sender` in the contract proves ownership of the token, and is sent directly from the `user`. We'll use `wagmi` frontend functions as well as some class composition to accomplish this.
