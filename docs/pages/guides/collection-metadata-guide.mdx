...
- [1. Obtain a Secret API Key](/guides/collection-metadata-guide#1-obtain-a-secret-api-key) from the [Sequence Builder Console](https://sequence.build)
- [2. Create Collection](/guides/collection-metadata-guide#2-create-collection-from-a-curl-request) from a cURL request one time
- [3. Create Token](/guides/collection-metadata-guide#3-create-token-using-tokenid) create a token using a tokenID
- [4. Create Asset](/guides/collection-metadata-guide#4-create-asset-using-tokenid) create an assetID
- [5. Store an Image](/guides/collection-metadata-guide#5-store-image-asset) process and store an image
- [6. Update to Non-private](/guides/collection-metadata-guide#6-update-non-private-token) update an asset to be non-private
- [7. Publish Collection](/guides/collection-metadata-guide#7-publish-collection-from-a-curl-request) from a cURL request one time
- [8. Render Asset from API](/guides/collection-metadata-guide#8-render-asset-from-api-publicly) from a cURL request one time
...
<Callout type="info">
    Full template code for this guide can be found [here](https://github.com/0xsequence-demos/template-cloudflare-worker-collections-api/tree/master)
</Callout>
...
myHeaders.append("Authorization", `Bearer ${process.env.JWT_ACCESS_KEY}`);
...
In the request, set the `metadadaField` (assetType) to `image`, with the other neccessary fields completed to return an asset response to be used in the next step
...
const jsonCreateAsset: any = await res2.json()
```
