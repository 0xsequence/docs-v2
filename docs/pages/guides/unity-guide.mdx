...
Jelly Forest is a blockchain-enabled 2D runner game. The game features social sign in, multi-tiered upgrades (with higher tier requiring lower tier upgrades as inputs to build/mint), and cosmetic upgrades, all of which are stored in an embedded non-custodial smart contract wallet.

Learn [why smart contract wallets here](https://docs-v2.sequence.xyz/solutions/technical-references/wallet-contracts/01-why)

Learn [what an embedded wallet is here](https://docs-v2.sequence.xyz/solutions/wallets/overview#what-are-the-differences-between-these-options)

// Todo add a video of Jelly Forest gameplay + shops

This guide will walk you through how we built Jelly Forest and how you too can build your own web3 game using [Sequence's Unity SDK](https://docs-v2.sequence.xyz/sdk/unity/01-overview/)!
...
## Integrate social sign in and Sequence's Embedded Wallet solution

### Configuration

1. Install Sequence's Unity SDK using package manager: https://docs-v2.sequence.xyz/sdk/unity/02-installation#package-manager---recommended
2. Sign into the Sequence [Builder Console](https://sequence.build/): https://docs-v2.sequence.xyz/solutions/builder/getting-started
3. Create a project for your game in the [Builder Console](https://sequence.build/): https://docs-v2.sequence.xyz/solutions/builder/project-management
4. Setup an Embedded Wallet in the [Builder Console](https://sequence.build/): // Todo insert link to docs
5. In your `SequenceConfig` [scriptable object](https://docs.unity3d.com/Manual/class-ScriptableObject.html) which you imported via the Samples menu in Package Manager during the [installation stage](https://docs-v2.sequence.xyz/sdk/unity/02-installation#package-manager---recommended), add your Google and Apple client ids which you added to the Builder as well as your Configuration Key under `WaaSConfigKey`
   - Don't forget to put your Android and iOS client ids under their respective platforms!
6. Add your `Builder API Key` from the [Builder Console](https://sequence.build/) under `Settings > API Access Keys` - you want the `prod` key
...
4. Create a UI manager to call `Open` on the `LoginPanel`. See our implementation here: https://github.com/0xsequence/sequence-unity-demo/blob/JellyForest/Scripts/UI/LoginScreenUIManager.cs
   - In our scene, this gets attached to the Canvas
...
### Authorization with the Sequence API

Once social sign in is complete, you will automatically make an authorization request with the Sequence WaaS (Wallet as a Service) APIs. Here's how it works:

> When social sign in is complete, the [OpenIdAuthenticator.SignedIn](https://github.com/0xsequence/sequence-unity/blob/master/Assets/SequenceSDK/Authentication/OpenIdAuthenticator.cs#L15) event is fired. This initiates the authorization process in [WaaSLogin.ConnectToWaaS](https://github.com/0xsequence/sequence-unity/blob/master/Assets/SequenceSDK/WaaS/WaaSLogin.cs#L217).

### Retrieving the user's wallet

In order to retrieve the wallet, you'll need to subscribe to the [WaaSWallet.OnWaaSWalletCreated](https://github.com/0xsequence/sequence-unity/blob/master/Assets/SequenceSDK/WaaS/WaaSWallet.cs#L16) event.
...
For more information on how auth in Sequence's Embedded Wallet solution works, please see our [docs](https://docs-v2.sequence.xyz/solutions/wallets/embedded-wallet/01-overview) and [blog post](https://sequence.xyz/blog/sequence-embedded-wallets).

## Deploy a collectibles contract

Now that our players can sign in and get a wallet, let's add some collectibles!

We highly recommend using an [ERC1155](https://eips.ethereum.org/EIPS/eip-1155) contract. They are a flexibly token standard that are well suited for games. You can easily deploy our audited ERC1155 implementation via the Builder Console like this: https://docs-v2.sequence.xyz/solutions/builder/contracts#watch-a-contract-be-deployed-with-builder. This is what we did for Jelly Forest.
...
Great! Now, when we send a POST request to our server with a body [defined in C#](https://github.com/0xsequence/sequence-unity/blob/Feature/transactionQueuer/Assets/SequenceSDK/Relayer/PermissionedMinter.cs#L93), where the `proof` is generated by the client sending the minting request. In the Unity SDK this is implemented by the [MintingRequestProver](https://github.com/0xsequence/sequence-unity/blob/Feature/transactionQueuer/Assets/SequenceSDK/Relayer/MintingRequestProver.cs#L29).

## Mint in-game tokens to the player's inventory

Now that we have our permissioned minter server setup, we need to get the client side (the Made With Unity app) hooked up so that we can start giving our players tokens through gameplay. We can send a request to the permissioned minter using the Unity SDK by calling the [PermissionedMinter.MintToken method](https://github.com/0xsequence/sequence-unity/blob/Feature/transactionQueuer/Assets/SequenceSDK/Relayer/PermissionedMinter.cs#L44).
...
Reading the tokens in a given user's wallet is a complicated process that is made much easier using [Sequence's Indexer](https://docs-v2.sequence.xyz/api/indexer/overview) which the [Unity SDK implements](https://docs-v2.sequence.xyz/sdk/unity/06-read-from-blockchain).
...
See it on [GitHub](https://github.com/0xsequence/sequence-unity-demo/blob/JellyForest/Scripts/Inventory.cs#L36)
...
When we first receive our `WaaSWallet` in Jelly Forest, [SequenceConnector](https://github.com/0xsequence/sequence-unity-demo/blob/JellyForest/Scripts/SequenceConnector.cs), which we used as our primary interface for communicating with the Sequence SDK in our game, [creates an `Inventory`](https://github.com/0xsequence/sequence-unity-demo/blob/JellyForest/Scripts/SequenceConnector.cs).
...
Sequence's Unity SDK provides a very flexible [transaction queueing system](https://github.com/0xsequence/sequence-unity/blob/Feature/transactionQueuer/Assets/SequenceSDK/Relayer/TransactionQueuer.cs).

In Jelly Forest, we attached a [PermissionedMinterTransactionQueuer](https://github.com/0xsequence/sequence-unity/blob/Feature/transactionQueuer/Assets/SequenceSDK/Relayer/PermissionedMinterTransactionQueuer.cs) MonoBehaviour to our `SequenceConnector` GameObject and [grab a reference to it in Awake](https://github.com/0xsequence/sequence-unity-demo/blob/JellyForest/Scripts/SequenceConnector.cs#L59). // Todo add screenshot of inspector

Once [this is setup](https://github.com/0xsequence/sequence-unity-demo/blob/JellyForest/Scripts/SequenceConnector.cs#L78), all we need to do when a token is collected [is call "mint token"](https://github.com/0xsequence/sequence-unity-demo/blob/JellyForest/Scripts/CollectibleToken.cs).
...
Our `TransactionQueuer`s can be configured to submit transactions automatically every X seconds, when promted (via function call) but no sooner than every Y seconds, or when prompted overriding any minimum time threshhold configured (Y seconds).
...
// Todo add a video of the editor extension here
...
All you have to do to bring your tokens into your game is check if the user owns enough of the given token id and apply the tokens effect.
