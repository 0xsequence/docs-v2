...
6. [Embedded Wallet Verification](/guides/linked-wallets-guide#6-embedded-wallet-verification): Reconstruct a message to be verified by the Sequence API to check the validity of an authenticated user session
...
<Callout type="info">
For this guide, in order to generate signatures a Parent Wallet will have to be deployed. You can learn about this [here](/solutions/wallets/embedded-wallet/examples/verification#prerequisite-deployment-of-a-parent-wallet)
</Callout>
...
And verify the `authProof` using the sequence api (installed with `pnpm install 0xsequence`), handle the case where the signature is not valid, and return the `walletAddress` when the signature is valid:
...
### Produce Verification Link for 2nd App

Once you have the variables required to verify a session, the variables are passed in a URL to redirect your users to a 2nd app, which will be parsed to verify a session so that an EOA can be used to sign in.

To construct the URL, use the base `VERIFICATION_URL` from [step 5](/guides/linked-wallets-guide#5-nonce-generation) link as the domain, and append the necessary variables using URL query params using the following code:
...
In the second dapp, follow the provided walkthrough [here](/sdk/sequence-kit/getting-started#getting-started-with-sequence-kit) to create an application that enables users to sign in with an Ethereum Owned Account (EOA) using Metamask, or, a Coinbase wallet, where the `VERIFICATION_URL` from [step 5](/guides/linked-wallets-guide#5-nonce-generation) links to with this 2nd app.
...
Then, the make a call to the Cloudflare Worker performed as:
...
The Embedded Wallet from the first application should be returned successfully.
