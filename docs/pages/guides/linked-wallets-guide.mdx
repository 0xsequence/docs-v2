...
In this guide, we will showcase how to sign into an Embedded Wallet using Web2 Auth, then, enable users to sign into an Externally-Owned Account (EOA) wallet in a different application to have the wallet linked to server storage with the Embedded Wallet account in an authorized frontend web session.
...
![wallet linking guide sequence diagram](/img/guides/wallet-linking/wallet_linking_guide_sequence_diagram.png)
...
<Callout type="info">
For this guide, while you're configuring your Embbedded Wallet, in order to generate signatures a Parent Wallet will have to be deployed. You can learn about this [here](/solutions/wallets/embedded-wallet/examples/verification#prerequisite-deployment-of-a-parent-wallet).
</Callout>
...
The Embedded Wallet from the first application should be returned successfully. After this, change the UI to accomodate the additional flow for signing in a user with an EOA wallet.
...
Great, you've created 1 application that sign's in a user with an Embedded Wallet and another application that is linked to from the first application that enables a user to sign into an EOA wallet, all tied together and authorized with a backend Cloudflare Worker. 
