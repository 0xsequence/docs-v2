...
# Embedded Wallet & EOA Linking Verification

Time to complete: XX-XX minutes

In this guide, we will showcase how to sign into an Embedded Wallet using Web2 Auth, then enable users to sign into an EOA wallet in a different application to have the wallet linked to server storage with the Embedded Wallet account. 

This feature is very useful for when users are being onboarded into an Embedded Wallet ecosystem, with prior history and assets in EOA wallets. The system can enable token-gating experiences to perform querying of inventory, leaving assets in respective wallets untouched.

<Callout type="info">
 The full code can be found in the respective repositories:
  - [Demo WaaS Auth](https://github.com/0xsequence/demo-waas-auth/blob/master/src/components/views/EOALinkView.tsx)
  - [Demo WaaS Wallet Link](https://github.com/0xsequence-demos/demo-waas-wallet-link)
Where the example can be tested from [this demo](https://0xsequence.github.io/demo-waas-auth/) at the bottom of the screen, from the card titled "EOA Link"
</Callout>

The Sequence tools in this guide will enable you to perform:

1. [Sequence Builder Console Signup & Project Creation](/guides/linked-wallets-guide#1-sequence-builder-console-signup--project-creation): Create a project with the Sequence Builder
2. [Access Key Management](/guides/linked-wallets-guide#2-access-key-management): Claim a public access key, and WaaS config key to interact with the Sequence stack
3. [Embedded Wallet Integration](/guides/linked-wallets-guide#3-embedded-wallet-integration): Integrate an Embedded Wallet into the application
4. [Deploy a Cloudflare Worker with KV Storage](/guides/linked-wallets-guide#4-deploy-a-cloudflare-worker-with-kv-storage): Leverage a serverless Cloudflare worker to hold state on wallet linking and wallet signature verification
5. [Nonce Generation](/guides/linked-wallets-guide#5-nonce-generation): Generate a signature directly from a signed in Embedded Wallet and generate and store a unique nonce 
6. [Embedded Wallet Verification](/guides/linked-wallets-guide#6-embedded-wallet-verification): Reconstruct a message to be verified by the Sequence API to check the validity of a authenticated user session
7. [Embedded Wallet Signature ](/guides/linked-wallets-guide#7-embedded-wallet-signature): Use Sequence Kit connector to sign a user in with their EOA wallet
8. [Sequence Kit Login for EOA Authentication](/guides/linked-wallets-guide#8-sequence-kit-login-for-eoa-authentication): Use Sequence Kit connector to sign a user in with their EOA wallet
9. [(Optional) Query Balances in Singular Interface](/guides/linked-wallets-guide#9-optional-query-balances-in-singular-interface): Use the Sequence Indexer to query multiple wallets to aggregate tokens into a singular interface
...
2. `Public Access Key` used for the Embedded Wallet, Sequence Kit, & Sequence Indexer, which can be done [here](/solutions/builder/getting-started#claim-an-api-access-key)
...
Then, the starter code is implemented with the previous imported files, in the following code within the `main.tsx` file, where the `GoogleOAuthProvider` wraps the application:
...
Finally, for this guide we will enable the system to store data in a Key-Value (KV) storage environment object. To start we will create a single variable that stores just a single nonce based on new requests.
...
Then in the rest of your code include the following testing code generate a valid nonce that is stored:
...
```shell
kv_namespaces = [
    { binding = "waas_wallet_link_db", id = "<YOUR_GENERATE_ID>" }
]
```
...
Now that we have an application that can log in a user, and a Cloudflare worker that can receive requests to generate a simple nonce, let create the structure of the worker to both generate a nonce for a wallet address, store it, and later verify the nonce using the Sequence API.
...
Returning to the backend, create a statement block with the following code to handle the path of `/verifyNonce` that checks that all variables are passed correctly:
...
```typescript
    let walletAddress = await env.waas_wallet_link_server_db.get(nonce)
...
And verify the `authProof` using the sequence api, handle the case the where the signature is not valid, and return the `walletAddress` when the signature is valid:
...
Great, the backend Cloudflare worker is complete, now it's time to create a Embedded Wallet Signature from the frontend to test verification works.
...
<Callout type="info"> 
The `network` value is the network name in plain text e.g. `mainnet`, `avalanche`, `base`, etc.
</Callout>
...
Then, call the `/verifyNonce` endpoint and pass with the variables of the `nonce` we generated prior, `sessionId` from the Embedded Wallet hook, `signature`, and `chainId` to your Cloudflare Worker, where the `chainId` is the number for the network you're dealing with:
...
## 8. Sequence Kit Login for EOA Authentication

In the second dapp, follow the provided walkthrough [here](/sdk/sequence-kit/getting-started#getting-started-with-sequence-kit) to create an application that enables users to sign in with an Ethereum Owned Account (EOA) using Metamask, or, a Coinbase wallet where the `VERIFICATION_URL` links to from [step 5](/guides/linked-wallets-guide#5-nonce-generation).

## 9. (Optional) Query Balances in Singular Interface
...
This can be accomplished with the following function and example:
...
const query = async () => {
    const res = await multiQuery(['0xC27F2c24daaf28161982f131e47AcF6734a04598', '0xd5Ba5624E48871B9CA7D73ec28451e0c0f1d0BdD']);
