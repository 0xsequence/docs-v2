...
Then, for the minting function to work programmatically so that transactions are gasless for your relayer, you will need to make the Transactions API draw from your account credits on your upgrade billing plan by sponsoring your deployed smart contract address.
...
When beginning your journey into AI image generation, you will require a source of model prompts to produce media. For this guide and demo, we've sourced prompts from the items contained in the [Diablo game](https://d07riv.github.io/diabloweb/)

In the [template](https://github.com/0xsequence-demos/demo-cloudflare-worker-treasure-chests/), we've included code to call an already deployed API and code to parse the response that is free to call yourself
...
### 2-Step Consent Process

Since each Dungeon Minter treasure chest opening undergoes a 2-step consent based process to minting, for every media generated, an adjacent image is stored to Sequence before actual minting.

In this consent flow, metadata is first stored, the `url` and `tokenID` is returned to the frontend, and the user later consents to minting the token where the `tokenID` with the `address` is passed back to the worker created in [Step 5](/guides/treasure-chest-guide#5-deploy-a-cloudflare-worker).

### Implementation

Complete and integrate [this guide](/guides/metadata-guide) in order to build your serverless Media Service that uses Cloudflare workers.

Cloudflare workers allows developers to run minimal infrastructure so that you can spend less time worrying about up time and server use.

Once complete, pass the stored media `tokenID` and `url` to the frontend:
...
To secure your Cloudflare worker to only process requests from a certain frontend origin, you can simply check the `request.headers` for the `Referrer` value and compare it against the `CLIENT_URL` in the `wrangler.toml`:
