...
Once the cloudflare worker has been deployed, you can call upon it from a frontend with the following code:
...
The complete process can be seen by this sequence diagram

![dungeon minter sequence diagram](/img/guides/treasure-chest/dungeon_minter_sequence_diagram.png)
...
In this consent flow, metadata is first stored, the `url` and the generated random `tokenID` (which allows for parallel requests) is returned to the frontend, and the user later consents after collectible inspection, to mint the token with the `tokenID` with the `address` passed back to the worker created in [Step 5](/guides/treasure-chest-guide#5-deploy-a-cloudflare-worker).
